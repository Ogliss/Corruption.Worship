        ...
        public override void LordToilTick()
        {
            if (Find.TickManager.TicksGame % 205 == 0)
            {
                ...
                for (int i = 0; i < this.lord.ownedPawns.Count; i++)
                {
                    ...
                    if (countArrived < (int)(this.lord.ownedPawns.Count * 0.8f) && !pawn.Position.InHorDistOf(data.dest, this.AllArrivedCheckRadius) || !pawn.CanReach(data.dest, PathEndMode.ClosestTouch, Danger.Deadly, false,  false, TraverseMode.ByPawn))
                    {
                        flag = false;
                        break;
                    }
