...
namespace Corruption.Worship
{
    public class GatheringWorker_Sermon : GatheringWorker
    {
        public override LordJob CreateLordJob(IntVec3 spot, Pawn organizer)
        {
            ...
            return new LordJob_Joinable_Sermon(altar, spot, organizer, this.def);
        }
        ...
        public override bool TryExecute(Map map, Pawn organizer = null)
        {
            ...
            // not sure what this was, it was commented out when i found it.
            // im guessing its for trying to get guests to join sermons, some chat history with ohu suggests so
        ...
        {
            return "AbilitySpeechStatInfo".Translate(organizer.Named("ORGANIZER"), StatDefOf.SocialImpact.label) + ": " + organizer.GetStatValue(StatDefOf.SocialImpact).ToStringPercent() + "\n\n" + "AbilitySpeechPossibleOutcomes".Translate() + ":\n" + (from o in LordJob_Joinable_Sermon.OutcomeChancesForPawn(organizer).Reverse() select o.Item1.stages[0].LabelCap + " " + o.Item2.ToStringPercent()).ToLineList("  - ");
        }
        public static bool AltarAccessible(BuildingAltar a, Pawn p)
        {
            return !a.SermonActive && p.CanReach(a.InteractionCell, PathEndMode.OnCell, p.NormalMaxDanger(), false, false, TraverseMode.ByPawn) && p.CanReserveSittableOrSpot(a.InteractionCell, false);
        }

        private static List<BuildingAltar> tmpAltars = new List<BuildingAltar>(); 
    }
    /*
    public class GatheringWorker_Sermon_V1 : GatheringWorker
    {
        public override LordJob CreateLordJob(IntVec3 spot, Pawn organizer)
        {
            BuildingAltar altar = spot.GetFirstThing<BuildingAltar>(organizer.Map);

            return new LordJob_Sermon(altar, new TargetInfo(altar), organizer, null, null, null, false);
        }
